 ...
 */
class Event : public dmn::Async {
public:
  Event() : dmn::Async{"event manager"} {}
...
int main(int argc, char *argv[]) {
  ...
  auto p2 = sortPipe.addTeePipeSource();
  dmn::Proc proc2{"s2", [&p2, &s2]() {
                    for (auto &v : s2) {
                      p2->write(v);
                    }
                  }};
  ...
  sortPipe.removeTeePipeSource(p1);
  sortPipe.removeTeePipeSource(p2);
  ...
  dmn::Pipe<std::string> pipe{"string"};
  ...
  auto pipeBlock = std::make_unique<dmn::Pipe<std::string>>("pipeBlock");
  dmn::Proc pipeBlockProc{"pipeBlockProc", [&pipeBlock]() {
