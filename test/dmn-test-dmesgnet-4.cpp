...
int main(int argc, char *argv[])
{
  ...
  std::unique_ptr<Dmn::Dmn_DMesgNet> dmesgnet1 = std::make_unique<Dmn::Dmn_DMesgNet>("dmesg1", readSocket1, writeSocket1);
  ...
  EXPECT_TRUE(dmesgPbLast.body().sys().self().masteridentifier() == "");
  EXPECT_TRUE(dmesgPbLast.body().sys().self().state() == Dmn::DMesgStatePb::Destroyed);
