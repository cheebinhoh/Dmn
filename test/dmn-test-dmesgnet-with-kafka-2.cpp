...
int main(int argc, char *argv[]) {
...

  {
    ...
    std::shared_ptr<dmn::Dmn_Kafka> producer = std::make_unique<dmn::Dmn_Kafka>(
    ...
    std::shared_ptr<dmn::Dmn_Kafka> consumer = std::make_unique<dmn::Dmn_Kafka>(
    ...
    while (true) {
      ...
      if (dataRead) {
        ...
        if (dmesgPbRead.body().sys().self().state() ==
