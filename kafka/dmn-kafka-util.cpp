...
set_config(rd_kafka_conf_t *config, std::string_view key, std::string_view value) {
  char errstr[KAFKA_ERROR_STRING_LENGTH]{};
  rd_kafka_conf_res_t res{};

  assert(nullptr != config || nullptr == "config parameter must not be nullptr");

  res = rd_kafka_conf_set(config, key.data(), value.data(), errstr, sizeof(errstr));
