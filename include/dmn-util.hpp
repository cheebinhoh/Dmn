 ...
 * and comparison with integer literal 1). For signed integer types, adding 1 to
 *    the maximum representable value is undefined behavior (signed overflow).
 * For unsigned integer types overflow is well-defined (modular arithmetic) and
 * the function will return 1 when wrapping occurs.
 *  - stringCompare makes copies of the provided string_views and applies a
 * simple ASCII-based lowercasing via ::tolower when case-insensitive comparison
 * is requested. This approach is locale-independent and does not perform full
 *    Unicode case folding — for locale-aware or Unicode-aware comparisons
 * prefer using std::collate/std::locale or a dedicated Unicode library.
 *
 * Complexity:
 *  - incrementByOne: O(1)
